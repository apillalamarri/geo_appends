#To use this state append script, you need to have python and pandas installed.

#This script will take an export from Revere Mobile, match it on zipcode to a file
#containing state information, and produce a file of just Mobile # and state
#as msisdn and 4f83b5a349f0237df0577896 (ID of "state" field on non-SEIU version of platform)
#If appending on SEIU, use 4f83b5a349f0237df0577899
...
#   python state_append.py export.csv
...
#Read US.txt into df_zip_info, add header row, then delete all but zipcode and state
...
df_zip_info = df_zip_info[['zipcode','state']]
...
#Keep only Mobile # and state, delete rows with no state
df_merged = df_merged[['Mobile #','state']]
df_merged = df_merged[df_merged.state.notnull()]

#Rename columns with Revere Mobile metadata IDs.
#If uploading to SEIU, use
#df_merged.columns = ['msisdn','4f83b5a349f0237df0577899'] #SEIU
df_merged.columns = ['msisdn','4f83b5a349f0237df0577896'] #Main

#Send dv_merged to a csv, eliminating the index
df_merged.to_csv(filename[0:-4]+'_state_append.csv',sep=',',index=False)
